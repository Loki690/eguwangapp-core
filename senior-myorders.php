...
if(isset($_SESSION['Role']) && $_SESSION['Role'] == "user"){
  ...
  $con_order = "SELECT * FROM orders WHERE `user_id` = '$session_id' AND `status` = '$o' ORDER BY `id` DESC";
...
<div class="topbar transition bg-primary fixed-top pb-5">
  ...
  <div class="content-start transition mt-5">
    ...
    <div class="content-header hstack g-3 mb-3">
        <h1>My Orders</h1> <span class="px-3 mx-3"><a href="senior-order-history.php">Purchase History</a></span>
    </div>
    <div class="row">
          ...
          <div class="card-body mt-2">
            <div class="row ${1| ,row-cols-2,row-cols-3, auto,justify-content-md-center,|}">
                ...
                <div class="col-md-12 mt-4 text-end border-bottom">
                  ...
                  <button class="btn btn-outline-primary"><span><i class='bx bxs-hand-up'></i></span> <?= $result['store_status'] ?></button>
                  <?php }elseif($result['store_status'] == "Preparing") {?>
                    <button class="btn btn-outline-warning"><span><i class='bx bxs-purchase-tag-alt'></i></span> <?= $result['store_status'] ?></button>
                    <?php }elseif($result['store_status'] == "Canceled"){ ?>
                      <button class="btn btn-outline-danger"><span><i class='bx bxs-package'></i></span> <?= $result['store_status'] ?></button>
                      <?php }else{ ?>
                        <button class="btn btn-outline-success"><span><i class='bx bxs-package'></i></span> <?= $result['store_status'] ?></button>
                        <?php } ?>
                  <a href="senior-order-details.php?id=<?= $result['id'] ?>"><button class="btn btn-primary">Check</button></a>
                </div>
            </div>
          </div>
