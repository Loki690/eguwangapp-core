...
<div class="sidebar transition overlay-scrollbars">
        <div class="sidebar-content"> 
            ...
            <ul class="side-menu">
				...
				<li>
					<a href="store-sales.php" class="active">
...
<table class="table table-border table-striped table-hover">
    ...
    <th scope="col">Prodcut Name</th>
        ...
        <th scope="col">No. Order Received</th>
        <th scope="col">Price</th>
        <th scope="col">Sales with 5% discount</th>
    ...
    <?php foreach($store_products as $item) { ?>
        ...
        // $info = $store->getProductinfo()

        $sales_q = "SELECT SUM(qty) AS QTY, total_price, SUM(total_price) AS TOTAL FROM `orders` WHERE `store_id` = '$store_id' AND `product_id` = '$p' AND `store_status` = 'Received'";
        $sales = $con->query($sales_q) or die($con->error);
        $sales_res = $sales->fetch_assoc();
        ...
        ?>
    <tr class="">
    <td scope="row"><?=  $item['product_name']. $p?></td>
    <td scope="row"><?=  $item['product_type']; ?></td>
    <!-- <td scope="row"><?=  array_sum($inventory_stocks); ?></td> -->
    <td scope="row"><?=  $sales_res['QTY'] ?></td>
    <td scope="row"><?=  "₱".$details['Price']; ?></td>
    <td scope="row"><?=  "₱".$sales_res['TOTAL']?></td>
